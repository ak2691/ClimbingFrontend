...
export default function UserPending() {
    ...
    useEffect(() => {
        const fetchPending = async () => {
            try {
                const res = await AuthFetch("http://localhost:8080/api/exercise-requests/my-pending", {
                    method: 'GET',
    ...
    const handleCancel = async () => {
        try {
            const res = await AuthFetch("http://localhost:8080/api/exercise-requests/cancel", {
                method: 'POST',
                headers: { 'content-type': 'application/json' },
                credentials: 'include'
            });
